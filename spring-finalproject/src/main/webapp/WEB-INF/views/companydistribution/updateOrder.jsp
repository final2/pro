		...
		<div class="container" style="margin-top:10px">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						<th>발주번호</th>
						<td colspan="2">${orders.no }<input type="hidden" name="orderNo" class="form-control" value="${orders.no }" /> </td>
						<th>발주신청일자</th>
					...
					<tr>
						<th>거래처명</th>
						<td colspan="2">${orders.client.name }<input type="hidden" name="clientName" class="form-control" value="${orders.client.name }" /></td>
						<th>입고확인 여부</th>
						<c:choose>
				...
				<tbody>
					...
					<c:forEach var="details" items="${details}">
						<tr>
							<td style="width:20%">${details.product.no }
								<input type="hidden" name="no" class="form-control" value="${details.product.no }" />
							</td>
							<td style="width:20%">${details.product.name }
							...
							<td style="width:20%"><fmt:formatNumber value="${details.product.price }" pattern="#,###"/> 
								<input type="hidden" name="price" class="form-control" value="${details.product.price}" />
							</td>
							<td style="width:20%" class="price"><fmt:formatNumber value="${details.product.price * 0.7 }" pattern="#,###"/></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
